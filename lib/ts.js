...
function compile (tsconfig, customTransformOptions = {
  ...
  let emitResult = program.emit(undefined, undefined, undefined, !!parsedCommandLine.options.emitDeclarationOnly, {
...
function createTransformer (isDeclarationFile, customTransformOptions, compilerOptions, program) {
  ...
  const convertModuleSuffix = (customTransformOptions.moduleSuffix !== ModuleSuffixKind.NONE) && (!!customTransformOptions.moduleSuffix)
  return (
    ...
    return (/** @type {ts.SourceFile} */ node) => ts.visitNode(node, visitor)
  }
}

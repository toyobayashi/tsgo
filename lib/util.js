...
const { existsSync, readFileSync, writeFileSync, copy } = require('fs-extra')
...
async function _spawn (command, args) {
  const cwd = getPath()
...
async function runNpmBin (bin, args) {
  ...
  if (existsSync(packageBin)) {
    return await _spawn(packageBin, args)
  }
...
async function useRollup (config) {
  ...
  await Promise.all(rollupConfig.map(conf => rollup(conf.input).then(bundle => bundle.write(conf.output))))
...
async function runTSC (tsconfigPath, watch, externalApiDeclarationDir, output) {
...
exports.watcher = {
  rollup (config) {
    const rollupConfig = [getRollupConfig(config, false), getRollupConfig(config, true)]
    require('rollup').watch(rollupConfig.map(conf => ({
      ...conf.input,
      output: conf.output,
  ...
  webpack (config) {
    ...
    require('webpack')(webpackConfig).watch({ aggregateTimeout: 200 }, (_err, stats) => console.log(stats.toString(webpackToStringOptions)))
  }
}

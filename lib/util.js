      ...
      plugins: [
        ...
        rollupReplace({
          ...((Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0) ? {
            ...
            '__DEV__': (format === 'esm' && !minify) ? '(process.env.NODE_ENV !== "production")' : !!minify,
  ...
  const webpackConfig = {
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ...(Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0 ? {
          'process.env.NODE_ENV': process.env.NODE_ENV !== undefined ? JSON.stringify(process.env.NODE_ENV) : '(undefined)',
          '__DEV__': (format === 'esm' && !minify) ? '(process.env.NODE_ENV !== "production")' : JSON.stringify(!!minify),
  ...
  if (Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0) {
    for (let i = 0; i < config.bundleOnly.length; i++) {
      ...
      if (typeof format === 'string') {
        configs.push(factory(config, false, format))
        configs.push(factory(config, true, format))
      } else if (typeof format === 'object' && format !== null) {
        configs.push(factory(config, format.minify, format.type))
      }
    }
  } else {
    configs.push(factory(config, false, 'umd'))
    configs.push(factory(config, true, 'umd'))
  }
  return configs
}

...
function getRollupConfig (config, minify, format = 'umd') {
  ...
  const outputFilename = minify ? getPath(config.output.rollup, `${config.output.name}${format === 'umd' ? '' : ('.' + format)}.min.js`) : getPath(config.output.rollup, `${config.output.name}${format === 'umd' ? '' : ('.' + format)}.js`)
  ...
  return {
    input: {
      ...
      plugins: [
        ...
        rollupReplace({
          ...((Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0) ? {
            'process.env.NODE_ENV': process.env.NODE_ENV !== undefined ? JSON.stringify(process.env.NODE_ENV) : '(undefined)',
            '__DEV__': format === 'esm' ? '(process.env.NODE_ENV !== "production")' : !!minify,
            '__GLOBAL__': (format === 'umd') || (format === 'iife')
          } : {}),
          ...(config.bundleDefine || {})
        }),
        rollupCommonJS({
...
async function useRollup (config) {
  ...
  if (Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0) {
    for (let i = 0; i < config.bundleOnly.length; i++) {
      ...
      rollupConfig.push(getRollupConfig(config, false, format))
      rollupConfig.push(getRollupConfig(config, true, format))
    }
  } else {
    rollupConfig.push(getRollupConfig(config, false, 'umd'))
    rollupConfig.push(getRollupConfig(config, true, 'umd'))
  }
...
function getWebpackConfig (config, minimize, format = 'umd') {
  ...
  const webpackConfig = {
    ...
    mode: 'production',
    entry: {
      [config.output.name + (format === 'umd' ? '' : ('.' + format)) + (minimize ? '.min' : '')]: [getPath(config.entry)]
    },
    output: {
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ...(Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0 ? {
          'process.env.NODE_ENV': process.env.NODE_ENV !== undefined ? JSON.stringify(process.env.NODE_ENV) : '(undefined)',
          '__DEV__': format === 'esm' ? '(process.env.NODE_ENV !== "production")' : JSON.stringify(!!minify),
          '__GLOBAL__': JSON.stringify((format === 'umd') || (format === 'iife'))
        } : {}),
        ...(config.bundleDefine || {})
      })
    ],
...
async function useWebpack (config) {
  ...
  if (Array.isArray(config.bundleOnly) && config.bundleOnly.length > 0) {
    for (let i = 0; i < config.bundleOnly.length; i++) {
      const format = config.bundleOnly[i]
      webpackConfig.push(getWebpackConfig(config, false, format))
      webpackConfig.push(getWebpackConfig(config, true, format))
    }
  } else {
    webpackConfig.push(getWebpackConfig(config, false, 'umd'))
    webpackConfig.push(getWebpackConfig(config, true, 'umd'))
  }

...
const { join, isAbsolute, basename, extname } = require('path')
...
function getRollupConfig (config, bundleOnlyCongig) {
  ...
  const input = getPath(config.entry)
  const isTypeScriptEntry = extname(input).indexOf('.ts') === 0
  ...
  return {
    input: {
      ...
      plugins: [
        ...
        ...(isTypeScriptEntry ? [rollupTypescript({
          tsconfig: getPath(config.tsconfig.esm)
        })] : []),
        rollupJSON(),
        ...
        ...(Array.isArray(config.externalRollupPlugins) ? [
          ...config.externalRollupPlugins
        ] : [])
      ]
    },
    output: {
...
function getWebpackConfig (config, bundleOnlyCongig) {
  ...
  const webpackConfig = {
    ...
    plugins: [
      ...
      ...(Array.isArray(config.externalWebpackPlugins) ? [
        ...config.externalWebpackPlugins
      ] : [])
    ],
    optimization: {
      minimize,
